...
public class ServiceTransactionEntryFinderImpl extends BasePersistenceImpl implements ServiceTransactionEntryFinder {
	...
	public final String SEARCH_ALL_SQL_ID = ServiceTransactionEntryFinderImpl.class.getName()+".searchAll";
	
	public final String SEARCH_COUNT_SQL_ID = ServiceTransactionEntryFinderImpl.class.getName()+".searchCount";
	public final String SEARCH_COUNT_ALL_SQL_ID = ServiceTransactionEntryFinderImpl.class.getName()+".searchCountAll";
	...
	public List<ServiceTransactionEntry> search(long transactionCode, long serviceId, long servicePackageId, int start, int end, int searchTypeFlag) 
		...
		try {
				...
				if (transactionCode == 0 && serviceId == 0 && servicePackageId == 0) {
					sql = CustomSQLUtil.get(SEARCH_ALL_SQL_ID);
	...
	public int searchCount(long transactionCode, long serviceId, long servicePackageId, int searchTypeFlag) throws SystemException {
		...
		try {
			...
			if (transactionCode == 0 && serviceId == 0 && servicePackageId == 0) {
				sql = CustomSQLUtil.get(SEARCH_COUNT_ALL_SQL_ID);
