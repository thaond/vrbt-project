...
<c:if test="<%= isAllow %>">
	...
	<liferay-ui:search-container delta="10" emptyResultsMessage="portlet-service_transaction_history-view-search-container-emptyResultsMessage">
		<liferay-ui:search-container-results 
			results="<%= Service_Transaction_HistoryExtLocalServiceUtil.search(transactionCodeNumber, 0, 0, searchContainer.getStart(), searchContainer.getEnd(), 1) %>"
			total="<%= Service_Transaction_HistoryExtLocalServiceUtil.searchCount(transactionCodeNumber, 0, 0, 1) %>"
		/>
		...
		<liferay-ui:search-container-row className="vn.com.fis.portal.portlet.util.model.Service_Transaction_HistoryExt" 
			...
			<% String transactionCode = "";
				...
				UserExt userExt = UserExtLocalServiceUtil.getUserExt(tran_history.getUserId());
				...
				try{
					if(serviceId != 0)
					serviceName = ServiceExtLocalServiceUtil.getServiceExt(serviceId).getServiceName();
				}catch(Exception e){e.printStackTrace();}
				long servicePackageId = tran_history.getServicePackageId();
				String servicePackageName = "";
				try{
				if(servicePackageId != 0)
					servicePackageName = Service_PackageExtLocalServiceUtil.getService_PackageExt(servicePackageId).getServicePackageName();
				}catch(Exception e){e.printStackTrace();}
			%>
